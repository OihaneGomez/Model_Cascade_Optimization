...
#Class to binarize obtained predictions and convert them to a binary array of N elements (being N the number of classes)
...
for item0,item_name in zip(alg_array0,alg_array_names): 

    print("--------------"+str(item_name))
    #Create reference model
    n_features=[0,featMax]
    model=item0
    #Obtain train data
    X_train,Y_train = train_process(wrist_class,n_features,model=1)
    #Normalize data
    ...
    #Obtain test data
    X_test,Y_test=prediction_process(wrist_class,n_features,model=1)
    files_test=gatherTest_names(wrist_class)

    #perform classification process file by file to see the performance of the model 
    error_count=0
    z=0
    for file_ in files_test:
        for f_ in file_:
            ...
            X_test=features_calculation_pred(data_x,data_y,data_z,data_gyrox,data_gyroy,data_gyroz,data_pitch,data_roll,data_yaw,data_split_x,data_split_y,data_split_z,data_split_gyrox,data_split_gyroy,
                                            data_split_gyroz,data_split_pitch,data_split_roll,data_split_yaw,n_features)
    ...
    timer = get_ipython().run_cell_magic('timeit', '-r 3 -n 10 -o ', '\nfor file_ in files_test:\n    for f_ in file_:\n        data_x,data_y,data_z,data_gyrox,data_gyroy,data_gyroz,data_pitch,data_roll,data_yaw,data_split_x,data_split_y,data_split_z,data_split_gyrox,data_split_gyroy,data_split_gyroz,data_split_pitch,data_split_roll,data_split_yaw=reformat_OnebyOne(f_) \n        X_test=features_calculation_pred(data_x,data_y,data_z,data_gyrox,data_gyroy,data_gyroz,data_pitch,data_roll,data_yaw,data_split_x,data_split_y,data_split_z,data_split_gyrox,data_split_gyroy,\n                                          data_split_gyroz,data_split_pitch,data_split_roll,data_split_yaw,n_features)\n        X_test= min_max_scaler.transform([X_test])\n        pred=model.predict(X_test)\n')
    ...
    with open('Prediction_task_times_1Model_MEAN.txt', 'w') as f:
        f.write(cap.stdout)    

    #Save data in .txt file, std values
    get_ipython().run_cell_magic('capture', 'cap ', 'print(array_dict_std)')
    with open('Prediction_task_times_1Model_STD.txt', 'w') as f:
        f.write(cap.stdout) 
